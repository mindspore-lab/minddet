...
def _update_run_op(
    beta1, beta2, eps, lr, weight_decay, param, m, v, gradient, decay_flag, optim_filter
):
    """
    Update parameters.
...
def _run_opt_with_sparse(
    ...
    if ps_parameter and not cache_enable:
        ...
        shapes = (
            ...
            op_shape(m),
            op_shape(v),
            op_shape(beta1_power),
            op_shape(beta2_power),
            op_shape(lr),
            op_shape(beta1),
            op_shape(beta2),
            op_shape(eps),
        ...
        success = F.depend(
            ...
            pull(
                push(
                    (beta1_power, beta2_power, lr, beta1, beta2, eps, values, indices),
                    shapes,
                ),
                param,
            ),
    ...
    if not target:
        ...
        next_m = scatter_add(
            m, grad_indices, op_mul(F.tuple_to_array((1.0,)) - beta1, grad_value)
        )
...
def _run_opt_with_one_number(
    ...
    if ps_parameter and not cache_enable:
        ...
        success = F.depend(
            ...
            pull(
                push(
                    (beta1_power, beta2_power, lr, beta1, beta2, eps, gradient),
...
def _run_off_load_opt(
    ...
    success = True
    delat_param = opt(
        moment1, moment2, beta1_power, beta2_power, lr, beta1, beta2, eps, gradient
    )
...
def _check_param_value(beta1, beta2, eps, prim_name):
    ...
    assert (
        isinstance(beta2, float) and 0.0 <= beta2 <= 1.0
    ), "beta2 should be float and between 0.0 and 1.0"
    assert isinstance(eps, float), "eps should be float and bigger than 0"
...
#     dt = F.dtype(grad)
#     if clip_type == 0:
#         new_grad = C.clip_by_value(grad, -1*clip_value, clip_value)
#     else:
...
class Adam(Optimizer):
    ...
    def __init__(
        ...
        self.beta1_power = Parameter(
            initializer(1, [1], mstype.float32), name="beta1_power"
        )
        self.beta2_power = Parameter(
            initializer(1, [1], mstype.float32), name="beta2_power"
        )
    ...
    def clip_grad_norm_(self, gradients):
        # clip_grad_norm_
        total_norm = 0.0
    ...
    def construct(self, gradients):
        ...
        # gradients = self.map_(F.partial(clip_grad, 0, 1 - 1e-4), gradients)
        gradients = self.clip_grad_norm_(gradients)
        gradients = self.decay_weight(gradients)
        gradients = self.gradients_centralization(gradients)
        gradients = self.scale_grad(gradients)
        gradients = self._grad_sparse_indices_deduplicate(gradients)
        lr = self.get_lr()
        beta1 = self.beta1[self.global_step]
        beta1_power = self.beta1_power * beta1
        ...
        if self.is_group:
            if self.is_group_lr:
                optim_result = self.hyper_map(
                    F.partial(_adam_opt, self.beta1, self.beta2, self.eps),
                    lr,
                    weight_decay,
                    self.parameters,
                    self.moments1,
            ...
            optim_result = self.hyper_map(
                F.partial(
                    _adam_opt, self.beta1, self.beta2, self.eps, lr, weight_decay
                ),
                self.parameters,
                self.moments1,
                self.moments2,
...
class AdamOffload(Optimizer):
    ...
    def __init__(
        ...
        super(AdamOffload, self).__init__(
