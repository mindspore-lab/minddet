...
int boxes_iou_bev_cpu(const float *boxes_a, int num_boxes_a,const float *boxes_b,int num_boxes_b, float *ans_iou) {
...
//int boxes_iou_nms_cpu(const float *boxes_a, int num_boxes_a,long *keep, float nms_overlap_thresh){
extern "C" int boxes_iou_nms_cpu(int nparam, void **params, int *ndims, int **shapes, const char **dtypes, void *stream, void *extra){
      if (nparam != 4) return 1;
      float *boxes_a = static_cast<float *>(params[0]);
      float *thresh = static_cast<float *>(params[1]);
      int *keep = static_cast<int *>(params[2]);
      int *out = static_cast<int *>(params[3]);
      float nms_overlap_thresh = *thresh;
      int num_boxes_a = 1000;
	  float areas[num_boxes_a];
	  int remove[num_boxes_a] = {0};
      for (int i = 0; i < num_boxes_a;i++) {
          keep[i] = 0;
      }
	  for (int i = 0; i < num_boxes_a;i++) {
        const float *boxa =  boxes_a + i * 7;
	  	areas[i] = boxa[3] * boxa[4];
		if (areas[i]==0) {
		    remove[i] = 1;
		}
	  }
	  for (int i = 0; i < num_boxes_a; i++) {
	      ...
	      for (int j = i + 1; j < num_boxes_a;j++) {
	          if (remove[j] == 1) {
	              continue;
	          }
	          const float *box_a = boxes_a + i * 7;
	          const float *box_b = boxes_a + j * 7;
	          float s_overlap = box_overlap(box_a, box_b);
	          float ovr = s_overlap / (areas[i] + areas[j] - s_overlap);
