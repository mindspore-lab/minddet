...
__device__ inline void rotate_around_center(const Point &center, const float angle_cos, const float angle_sin,
                                            Point &p) {
...
__global__ void boxes_overlap_kernel(const int num_a, const float *boxes_a, const int num_b, const float *boxes_b,
                                     float *ans_overlap) {
...
__global__ void boxes_iou_bev_kernel(const int num_a, const float *boxes_a, const int num_b, const float *boxes_b,
                                     float *ans_iou){
    // params boxes_a: (N, 7) [x, y, z, dx, dy, dz, heading]
    // params boxes_b: (M, 7) [x, y, z, dx, dy, dz, heading]
    const int a_idx = blockIdx.y * THREADS_PER_BLOCK + threadIdx.y;
...
__global__ void nms_kernel(const int boxes_num, const float *nms_overlap_thresh,
    ...
    if (threadIdx.x < row_size) {
        ...
        for (i = start; i < col_size; i++) {
            if (iou_bev(cur_box, block_boxes + i * 7) > nms_overlap_thresh[0]){
                t |= 1ULL << i;
            }
        }
...
__global__ void nms_normal_kernel(const int boxes_num, const float *nms_overlap_thresh,
    ...
    if (threadIdx.x < row_size) {
        ...
        for (i = start; i < col_size; i++) {
            if (iou_normal(cur_box, block_boxes + i * 7) > nms_overlap_thresh[0]){
                t |= 1ULL << i;
            }
        }
        const int col_blocks = DIVUP(boxes_num, THREADS_PER_BLOCK_NMS);
...
extern "C" int BoxesIouBevGpu(int nparam, void** params, int* ndims, int64_t** shapes, const char** dtypes,
    void* stream, void* extra)
{
    cudaStream_t custream = static_cast<cudaStream_t>(stream);
    cudaStreamSynchronize(custream);
    auto tensors = get_torch_tensors(nparam, params, ndims, shapes, dtypes, c10::kCUDA);
    auto boxes_a = tensors[0];
    auto boxes_b = tensors[1];
    auto ans_iou = tensors[2];
...
extern "C" int BoxesOverlapBevGpu(int nparam, void** params, int* ndims, int64_t** shapes, const char** dtypes,
    void* stream, void* extra)
{
    cudaStream_t custream = static_cast<cudaStream_t>(stream);
    cudaStreamSynchronize(custream);
...
// nms_gpu
extern "C" int NmsGpu(int nparam, void** params, int* ndims, int64_t** shapes, const char** dtypes,
    void* stream, void* extra)
{
    cudaStream_t custream = static_cast<cudaStream_t>(stream);
    cudaStreamSynchronize(custream);
    auto tensors = get_torch_tensors(nparam, params, ndims, shapes, dtypes, c10::kCUDA);
    auto boxes = tensors[0];        // boxes  float        tensor [N, 7]
    auto thresh = tensors[1];       // thresh float        tensor [1]
    auto keep = tensors[2];         // keep   long(int64)  tensor [N]
    auto num_to_keep = tensors[3];  // num_to_keep int     tensor [1]

    int boxes_num = boxes.size(0);
    const float *boxes_data = boxes.data<float>();     // gpuptr    float tensor
    float *nms_overlap_thresh = thresh.data<float>();  // gpuptr    float tensor
    long *keep_data_gpu = keep.data<long>();           // keep out1 long tensor gpuptr
    int *num_to_keep_gpu = num_to_keep.data<int>();    // num  out2 int         gpuptr
    ...
    long keep_cpu[keep.size(0)];                // tmpkeep cpuptr
    memset(keep_cpu, 0, keep.size(0) * sizeof(long));
    int num_to_keep_cpu = 0;

    for (int i = 0; i < boxes_num; i++){
        int nblock = i / THREADS_PER_BLOCK_NMS;
        int inblock = i % THREADS_PER_BLOCK_NMS;
        if (!(remv_cpu[nblock] & (1ULL << inblock))){
            keep_cpu[num_to_keep_cpu++] = i;    // need use cpuptr to receive i and then copy to gpuptr
    ...
    CHECK_ERROR(cudaMemcpy(keep_data_gpu, keep_cpu, keep.size(0) * sizeof(long), cudaMemcpyHostToDevice));
    CHECK_ERROR(cudaMemcpy(num_to_keep_gpu, &num_to_keep_cpu, sizeof(int), cudaMemcpyHostToDevice));
...
// nms_normal_gpu
extern "C" int NmsNormalGpu(int nparam, void** params, int* ndims, int64_t** shapes, const char** dtypes,
    void* stream, void* extra)
{
    cudaStream_t custream = static_cast<cudaStream_t>(stream);
    cudaStreamSynchronize(custream);
    ...
    int boxes_num = boxes.size(0);
    const float *boxes_data = boxes.data<float>();     // gpuptr
    float *nms_overlap_thresh = thresh.data<float>();  // gpuptr
    long *keep_data_gpu = keep.data<long>();           // keep out1 long tensor gpuptr
    int *num_to_keep_gpu = num_to_keep.data<int>();    // num  out2 int         gpuptr
