...
from det3d_ms.core.bbox.geometry import (
                ...
                if ih > 0:
                    for k in range(4):
                        for lm in range(4):
                            A = lines_boxes[i, k, 0]
                            B = lines_boxes[i, k, 1]
                            C = lines_qboxes[j, lm, 0]
                            D = lines_qboxes[j, lm, 1]
                    ...
                    if ret[i, j] is False:
                        ...
                        for lm in range(4):  # point lm in qboxes
                            for k in range(4):  # corner k in boxes
                                vec = boxes[i, k] - boxes[i, (k + 1) % 4]
                                if clockwise:
                                    vec = -vec
                                cross = vec[1] * (boxes[i, k, 0] - qboxes[j, lm, 0])
                                cross -= vec[0] * (boxes[i, k, 1] - qboxes[j, lm, 1])
                                if cross >= 0:
                        ...
                        if box_overlap_qbox is False:
                            ...
                            for lm in range(4):  # point lm in boxes
                                for k in range(4):  # corner k in qboxes
                                    vec = qboxes[j, k] - qboxes[j, (k + 1) % 4]
                                    if clockwise:
                                        vec = -vec
                                    cross = vec[1] * (qboxes[j, k, 0] - boxes[i, lm, 0])
                                    cross -= vec[0] * (
                                        qboxes[j, k, 1] - boxes[i, lm, 1]
                                    )
                                    if cross >= 0:  #
