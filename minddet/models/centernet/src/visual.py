...
from .image import affine_transform, get_affine_transform
...
def resize_image(image, anns, width, height):
    ...
    out_img = cv2.warpAffine(
        image, trans_output, (width, height), flags=cv2.INTER_LINEAR
    )
...
def visual(
    ann_path, image_path, save_path, ratio=1, mode="val", name="merged_annotations"
):
...
def visual_image(
    ...
    for i in range(num_objects):
        ...
        if "score" in ann:
            ...
            cv2.rectangle(
                img,
                (bbox[0], int(bbox[1] - cat_size[1] - 5)),
