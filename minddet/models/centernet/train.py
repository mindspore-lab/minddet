...
def _get_optimizer(network, dataset_size):
    ...
    if train_config.optimizer == "Adam":
        ...
        if train_config.lr_schedule == "PolyDecay":
            ...
            raise ValueError(
                "Don't support lr_schedule {}, only support [PolynormialDecay, MultiEpochDecay]".format(
                    train_config.optimizer
                )
            )
    else:
        raise ValueError(
            "Don't support optimizer {}, only support [Lamb, Momentum, Adam]".format(
                train_config.optimizer
            )
        )
...
def modelarts_pre_process():
    ...
    config.save_checkpoint_path = os.path.join(
        config.output_path, config.save_checkpoint_path
    )
...
def train():
    ...
    if config.device_target == "Ascend":
        ...
        if config.distribute == "true":
            ...
            rank = get_rank_id()
            ckpt_save_dir = (
                config.save_checkpoint_path + "ckpt_" + str(get_rank()) + "/"
            )
    ...
    if config.device_target == "Ascend":
        net_with_grads = CenterNetWithLossScaleCell(
            net_with_loss, optimizer=optimizer, sens=train_config.loss_scale_value
        )
    else:
        net_with_grads = CenterNetWithoutLossScaleCell(
