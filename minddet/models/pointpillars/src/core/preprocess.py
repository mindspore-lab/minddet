...
@numba.njit
def group_transform_(loc_noise, rot_noise, locs, group_center, valid_mask):
    """group transform"""
...
def group_transform_v2_(
    loc_noise, rot_noise, locs, group_center, grot_noise, valid_mask
):
    """group transform v2"""
    # loc_noise: [N, M, 3], locs: [N, 3]
    # rot_noise: [N, M]
...
def noise_per_object(
    ...
    if group_ids is not None:
        ...
        if enable_grot:
            ...
            group_transform_(
                loc_noises, rot_noises, gt_boxes[:, :3], group_centers, valid_mask
            )
        ...
        for j in range(k_qboxes):
            ...
            if ih > 0 and iw > 0:
                ...
                if ret[i, j] is False:
                    ...
                    # box overlap qbox:
                    box_overlap_qbox = _get_box_overlap_another(
                        boxes, qboxes, clockwise, i, j
                    )

                    if box_overlap_qbox is False:
                        qbox_overlap_box = _get_box_overlap_another(
                            qboxes, boxes, clockwise, j, i
                        )
