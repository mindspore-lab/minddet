...
def create_anchors_3d_stride(
    ...
    rotations=(0, np.pi / 2),
    anchor_range=[0.0, -39.68, -3.0, 69.12, 39.68, 1.0],
    dtype=np.float32,
):
    """
    Args:
        feature_size: list [D, H, W](zyx) 1 248 216
    ...
    num_anchor_size, num_anchor_rotation = 1, 2
    anchor_rotation = np.array(rotations, dtype=dtype)
    anchor_size = np.reshape(np.array(sizes, dtype=dtype), [-1, 3])
    x_shifts, y_shifts, z_shifts = ms.ops.meshgrid(
        ms.Tensor.from_numpy(x_shifts),
        ms.Tensor.from_numpy(y_shifts),
        ms.Tensor.from_numpy(z_shifts),
        indexing="ij",
    )  # [x_grid, y_grid, z_grid]
    anchors = np.stack(
        (x_shifts.asnumpy(), y_shifts.asnumpy(), z_shifts.asnumpy()), axis=-1
    )  # [x, y, z, 3]
    anchors = np.tile(anchors[:, :, :, None, :], (1, 1, 1, anchor_size.shape[0], 1))
    anchor_size = np.tile(
        anchor_size.reshape(1, 1, 1, -1, 3), ([*anchors.shape[0:3], 1, 1])
    )
    anchors = np.concatenate((anchors, anchor_size), axis=-1)
    anchors = np.tile(
        anchors[:, :, :, :, None, :], (1, 1, 1, 1, num_anchor_rotation, 1)
    )
    anchor_rotation = np.tile(
        anchor_rotation.reshape(1, 1, 1, 1, -1, 1),
        ([*anchors.shape[0:3], num_anchor_size, 1, 1]),
    )

    anchors = np.concatenate((anchors, anchor_rotation), axis=-1)
    anchors = np.transpose(anchors, [2, 1, 0, 3, 4, 5])
...
def box_camera_to_lidar(data, r_rect, velo2cam, if_self_train=True):
    """box camera to lidar"""
    # if not if_self_train:
    #     boxes3d_camera_copy = copy.deepcopy(data)
    #     xyz = boxes3d_camera_copy[:, 0:3]
    #     l, h, w = boxes3d_camera_copy[:, 3:4], boxes3d_camera_copy[:, 4:5], boxes3d_camera_copy[:, 5:6]
    #     r = boxes3d_camera_copy[:, 6:7]
