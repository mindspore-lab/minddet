...
def indices_to_dense_vector(indices, size, indices_value=1.0, default_value=0):
...
def _sigmoid_cross_entropy_with_logits(logits, labels):
    ...
    loss = ops.clip_by_value(
        logits, clip_value_min=_create_off_value(), clip_value_max=logits.max()
...
def _softmax_cross_entropy_with_logits(logits, labels):
    ...
    loss = loss_ftor(
        logits.astype(mstype.float32),
        ops.ArgMaxWithValue(axis=-1)(labels)[0].astype(mstype.int32),
    )
    return loss
...
class SigmoidFocalClassificationLoss(nn.Cell):
    ...
    def construct(self, prediction_tensor, target_tensor, weights, class_indices=None):
        ...
        if class_indices is not None:
            weights *= (
                ...
                .view(1, 1, -1)
                .astype(prediction_tensor.dtype)
            )
...
class WeightedSmoothL1LocalizationLoss(nn.Cell):
    ...
    def construct(self, prediction_tensor, target_tensor, weights=None):
        ...
        abs_diff_lt_1 = ops.LessEqual()(abs_diff, 1 / (self._sigma**2)).astype(
            abs_diff.dtype
        )
        loss = abs_diff_lt_1 * 0.5 * ops.Pow()(abs_diff * self._sigma, 2) + (
            abs_diff - 1 / (2 * (self._sigma**2))
        ) * (1.0 - abs_diff_lt_1)
        if self._codewise:
